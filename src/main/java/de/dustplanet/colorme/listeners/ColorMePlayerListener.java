...
/**
 * ColorMe for CraftBukkit/Bukkit Handles the player activities
 * 
 * Refer to the forum thread: http://bit.ly/colormebukkit Refer to the
 * dev.bukkit.org page: http://bit.ly/bukkitdevcolorme
 * 
...
public class ColorMePlayerListener implements Listener {
	...
	if (plugin.newColorOnJoin) {
	    ...
	    // 22 == Reset -> bad
	    while (color == 22) {
		color = (int) (Math.random() * ((colors + 2 + customColors)));
	    }
	    // Set it.
	    if (color == ChatColor.values().length + 1) {
		actions.set(name, "rainbow", world, pluginPart[0]);
	    } else if (color == ChatColor.values().length + 2) {
		actions.set(name, "random", world, pluginPart[0]);
	    } else if (color > ChatColor.values().length + 2) {
		color -= (2 + ChatColor.values().length);
		int i = 0;
		for (String colorName : plugin.colors.getKeys(false)) {
		    if (i == color) {
			actions.set(name, colorName, world, pluginPart[0]);
		    }
		    i++;
		}
	    } else {
	...
	if (plugin.groups) {
		...
		// World data -> then groups (only first) and finally the suffix
		// and prefix!
		...
		if (actions.playerHasGroup(name)) {
		    ...
		    if (actions.hasGroup(group, world, "prefix")) {
			groupPrefix = actions.getGroup(group, world, "prefix");
		    }
		    if (actions.hasGroup(group, "default", "prefix")) {
			groupPrefix = actions.getGroup(group, "default", "prefix");
		    }
		    if (actions.hasGroup(group, world, "suffix")) {
			groupSuffix = actions.getGroup(group, world, "suffix");
		    }
		    if (actions.hasGroup(group, "default", "suffix")) {
			groupSuffix = actions.getGroup(group, "default", "suffix");
		    }
		}
	    }
	}
	...
	if (!plugin.otherChatPluginFound) {
		...
		if (!plugin.chatBrackets) {
			...
			format = globalPrefix + ChatColor.RESET + groupPrefix
				+ ChatColor.RESET + prefix + ChatColor.RESET
				+ "%1$s" + ChatColor.RESET + groupSuffix
				+ ChatColor.RESET + suffix + ChatColor.RESET
				+ globalSuffix + ": %2$s";
		    } else {
			format = globalPrefix + ChatColor.RESET + groupPrefix
				+ ChatColor.RESET + prefix + ChatColor.RESET
			...
			event.setFormat("[FACTION] " + format);
		    }
		} else {
		    if (!globalSuffix.equals("")) {
			globalSuffix += ChatColor.RESET + ": ";
		    } else if (!suffix.equals("")) {
			suffix += ChatColor.RESET + ": ";
		    } else if (!groupSuffix.equals("")) {
			groupSuffix += ChatColor.RESET + ": ";
		    }
		    if (!groupSuffix.equals("") && !suffix.equals("")) {
			groupSuffix += " ";
		    }
		    if (!suffix.equals("") && !globalSuffix.equals("")) {
			suffix += " ";
		    }
		    format = globalPrefix + ChatColor.RESET + groupPrefix
			    + ChatColor.RESET + prefix + ChatColor.RESET
			    + "<%1$s" + ChatColor.RESET + "> "
			    + ChatColor.RESET + groupSuffix + ChatColor.RESET
			    + suffix + ChatColor.RESET + globalSuffix + "%2$s";
		    if (!plugin.factions) {
			event.setFormat(format);
		    } else {
			event.setFormat("[FACTION] " + format);
