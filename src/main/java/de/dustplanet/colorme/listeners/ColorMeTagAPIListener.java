...
public class ColorMeTagAPIListener implements Listener {
	...
	if (event.getNamedPlayer().hasPermission("colorme.nametag")) {
		...
		if (plugin.ownSystem) {
			...
			if (actions.hasGroup(group, world, "colors")) {
			    color = actions.getGroup(group, world, "colors");
			}
			// Group default
			else if (actions.hasGroup(group, "default", "colors")) {
	    ...
	    if (plugin.globalColor && color == null && actions.hasGlobal("color")) {
		...
		if (!actions.isStandard(colorActual)) {
		    if (colorActual.equalsIgnoreCase("rainbow")) {
			...
			// No section sign or ampersand? Not valid; doesn't
			// start with a section sign? Not valid! Ending without
			// a char? Not valid!
			...
			// Split the color values
			    ...
			    // Special here, since we already have got the
			    // section sign sign!
